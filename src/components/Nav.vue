<script setup lang="ts">
  ...
  const showSidebarContent = (content: string)=>{
    currentComponent.value = !currentComponent.value
    currentSidebarContent.value = content
    console.log(currentSidebarContent.value)
  }
</script>
<template>
      ...
      <div class="flex items-center mt-2 grow sm:mt-0 sm:mr-6 md:mr-0 lg:flex lg:basis-auto">
        ...
        <ul class="flex flex-row justify-end pl-0 mb-0 list-none md-max:w-full">
          ...
          <li class="relative flex items-center pr-2">
            ...
            <button
              @click="showSidebarContent('notifications')"
            ...
            <Notifications v-if="currentComponent && currentSidebarContent === 'notifications'"/>
          </li>
          <li class="relative flex items-center pr-2">
            <p class="hidden transform-dropdown-show"></p>
            <button 
              @click="showSidebarContent('user')"
              class="block p-0 transition-all text-sm ease-nav-brand text-slate-500" 
            >
              <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-6 h-6">
