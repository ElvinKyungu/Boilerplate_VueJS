<script lang="ts" setup>
  ...
  const handleEmailBlur = () => {
    if (!userLogin.value.email.match(/^[^\s@]+@[^\s@]+\.[^\s@]+$/)) {
  ...
  const handlePasswordBlur = () => {
    if (userLogin.value.password.length < 6) {
  ...
  const handlLogin = async()=>{
    if(!emailError.value && !passwordError.value){
      try {
        const result = await signinWithPassword(userLogin.value)
        console.log(result)
      } catch (error) {
...
<template>
  <form>
    ...
    <div class="mt-8">
      ...
      <span class="text-red-500 ml-4 mt-2 block" v-if="passwordError !== ''">
        {{ passwordError }}
      </span>
    </div>
